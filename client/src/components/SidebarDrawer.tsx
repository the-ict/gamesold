  ...
  return (
    <div className={`${openAnimation ? 'sidebar-drawer' : ''} ${closeAnimation ? 'animate-slide-exit' : ''} fixed top-0 ${sidebarSide} ${sidebarWidth} h-full ${sidebarBackgroundColor} text-black shadow-lg z-50 p-5 overflow-y-auto`}>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-bold text-white">{sidebarName}</h2>
        <CgClose className="text-gray-300 cursor-pointer hover:text-white transition-colors duration-300" size={24} onClick={() => setSidebarOpen(false)} />
      </div>
      {children}
    </div>
  )
